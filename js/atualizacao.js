...
function setClass(className) {
    ...
    if (alertDiv) {
        alertDiv.textContent = `Turma selecionada: ${className}`;
...
function displaySavedData() {
    ...
    students.forEach((student, index) => {
        ...
        userCard.innerHTML = `
            <div>
                <p class="user-name">#${index + 1} ${student.nomeFormatado}</p>
                <p class="user-coins ${student.coins < 0 ? 'negative' : ''}">
                    MKR: ${student.coins.toLocaleString()}
                </p>
            </div>
            <button class="crypto-button danger" onclick="deleteData('${student.key}')">
                <i class="fas fa-trash"></i>
                Excluir
            </button>
        `;
        savedDataContainer.appendChild(userCard);
    });
    ...
    if (students.length === 0) {
        savedDataContainer.innerHTML = `
            <div class="empty-state">
                ...
                <small>Importe uma lista ou adicione estudantes individualmente</small>
            </div>
        `;
    }
    
    addTerminalLog(`Exibindo ${totalNomes} estudantes da turma ${selectedClass}`, 'info');
}
