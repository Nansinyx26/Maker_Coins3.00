...
async function saveData() {
  ...
  try {
    ...
    await apiClient.adminUpdateCoins({
      email: user.email,
      valor: coins,
      descricao: isNegative ? justificativa : 'Crédito administrativo 2º Ano',
      secret: 'Mantra2222'
    });
...
function exportToTXT() {
  ...
  link.setAttribute("download", `turma_${selectedClass}_2ano.txt`);
  link.click();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Turma padrão 2A no carregamento
  setClass('2A');
});
